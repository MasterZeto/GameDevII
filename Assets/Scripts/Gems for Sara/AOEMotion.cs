  ...
  //  LineRenderer lr;
    ...
    void Update()
    {
        if (fighter.pause != true)
        {
            ...
            time += 55 * Time.deltaTime;
            lz = transform.forward.z - time * speed;

            //  float y = transform.position.y + z * Mathf.Tan(radianAngle) - ((g * z * z) / (2 *30f *30f * Mathf.Cos(radianAngle) * Mathf.Cos(radianAngle)));
            ly = transform.forward.y + time * speed * Mathf.Tan(radianAngle) - ((g * time * speed * time * speed) / (0.1f * Mathf.Cos(radianAngle) * Mathf.Cos(radianAngle)));
            lx = transform.forward.x;
